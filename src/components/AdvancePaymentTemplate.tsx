...
const AdvancePaymentTemplate: React.FC<Props> = ({ submodule, company }) => {
  return (
    <div className="min-h-screen bg-slate-100 py-6 px-2 sm:py-10 sm:px-4">
      <div className="max-w-3xl mx-auto bg-white border shadow rounded-lg overflow-hidden print:shadow-none print:border-none">
        {/* Header */}
        <div className="px-6 py-6 flex flex-col sm:flex-row justify-between items-start sm:items-center border-b gap-4 bg-indigo-50">
          ...
          <div className="text-left sm:text-right">
            ...
            <p className="text-sm text-gray-700">
              AP No: {submodule?.payment_number}
            </p>
            <p className="text-sm text-gray-700">
              Date: {formatDate(submodule?.date_timestamp)}
            </p>
            <p className="text-sm capitalize font-medium text-indigo-700">
              Status: {(submodule?.payment_status).toUpperCase() || "-"}
            </p>
          </div>
        </div>
        ...
        <div className="px-6 py-4 text-center bg-gray-50 print:hidden">
          <button
            ...
            className="px-6 py-2 cursor-pointer bg-indigo-700 text-white rounded text-sm font-medium hover:bg-indigo-800"
          >
...
export default AdvancePaymentTemplate;
